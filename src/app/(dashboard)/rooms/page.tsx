      ...
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <button
            ...
            className={clsx(
              'w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-left transition border-l-2 italic',
              activeRoomId === UNCATEGORISED_ID
                ? 'bg-gray-50 border-l-gray-400 text-gray-700 font-medium'
          ...
          {/* MiniCRM bootstrap: csak admin/CEO látja. Létrehozza a 9 "MiniCRM…"
              ...
              (mini_crm_id + miniCrmBootstrapDone flag). Szoba-szintű projekt-
              import egy KÜLÖN gombbal jön minden szobánál.

              A korábbi "+ Drón szoba importálása" + "Repülések → szobák" gombok
              eltávolítva — a MiniCRM Légtér ügyintézés és sales szoba és
    ...
    <section className="flex-1 flex flex-col bg-gray-50 min-w-0">
      <header className="px-6 py-4 border-b border-gray-100 bg-white flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* MiniCRM szoba-szintű projekt-import: csak ADMIN/CEO látja és csak
              ha a szoba "MiniCRM …" prefixű. Az ide tartozó modul-CSV-ből
              importálja a projekteket + a kapcsolódó task-okat. */}
          {category && category.name.startsWith('MiniCRM ') && (
            <MiniCrmImportSzobaButton categoryId={category.id} categoryName={category.name} onDone={() => { mutate(); bumpOverview(); }} />
          )}
          <button
...
// ─── MiniCRM bootstrap gomb (admin-only) ─────────────────────────────────────

function MiniCrmImportButton({ onDone }: { onDone: () => void }) {
  const { currentUser } = useCurrentUser();
  const [busy, setBusy] = useState(false);
  const [confirm, setConfirm] = useState(false);
  // Ha a backend `skipped: already-done`-t ad (a tenant.feature_flags.miniCrmBootstrapDone
  // flag már true), az admin a megjelenő "Újra futtatás (force)" gombbal felülbírálhatja.
  ...
  async function runImport(force = false) {
    ...
    try {
      const r = await apiClient.post('/minicrm/import/bootstrap-szobak', force ? { force: true } : {});
      const stats = r.data?.stats ?? {};
      const skipped = r.data?.skipped;
      if (skipped) {
        toast.info(`MiniCRM import: ${skipped} — már le volt futtatva.`);
        setForceConfirm(true);
      } else {
        const summary = Object.entries(stats).map(([k, v]) => `${k}: ${v}`).join(', ');
        toast.success(`MiniCRM import kész — ${summary}`);
      }
      onDone();
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? 'MiniCRM import sikertelen');
    } finally {
      setBusy(false);
    }
  }
  ...
  if (forceConfirm) {
    return (
      <div className="px-4 py-2.5 bg-red-50 border-l-2 border-l-red-500 space-y-2">
        <p className="text-xs text-red-900">
          A globális bootstrap <strong>már le volt futtatva</strong> ezen a tenant-on. Ha tényleg
          újra le akarod futtatni (pl. új MiniCRM-export feltöltése után), kattints az
          <strong> „Újra futtatás (force)"</strong>-ra. Idempotens — a meglévő rekordokat (felhasználók,
          fiókok, ügyfelek) <strong>nem duplikálja</strong> a mini_crm_id alapján.
        </p>
        <div className="flex gap-2">
          <button
            onClick={() => runImport(true)}
            disabled={busy}
            className="flex-1 px-3 py-1.5 rounded text-xs font-medium bg-red-600 text-white hover:bg-red-700 disabled:opacity-50"
          >
            {busy ? 'Importálás…' : 'Újra futtatás (force)'}
          </button>
          <button
  ...
  if (confirm) {
    return (
      <div className="px-4 py-2.5 bg-amber-50 border-l-2 border-l-amber-400 space-y-2">
        <p className="text-xs text-amber-900">
          Létrejön a 9 új <strong>"MiniCRM…"</strong> szoba + globális adatok (felhasználók, fiókok,
          ügyfelek). Ez a meglévő csapat-adatokat <strong>nem</strong> érinti, idempotens (csak egyszer
          fut effektíven). Pár perc.
        </p>
        <div className="flex gap-2">
          <button
            onClick={() => runImport(false)}
            disabled={busy}
            className="flex-1 px-3 py-1.5 rounded text-xs font-medium bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50"
          >
            {busy ? 'Importálás…' : 'Indítás'}
          </button>
          <button
  ...
  return (
    <button
      ...
      disabled={busy}
      title="A MiniCRM-exportból importálja a 9 új szobát + globális alapadatokat (felhasználók, fiókok, ügyfelek). Csak admin/CEO látja."
      className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-left transition border-l-2 border-l-transparent text-gray-500 hover:bg-gray-50 disabled:opacity-50"
    >
      <Download className="w-4 h-4 shrink-0" />
      <span className="flex-1 truncate">MiniCRM szobák importálása</span>
    </button>
  );
}

// ─── MiniCRM szoba-szintű projekt-import gomb (admin-only) ────────────────────

function MiniCrmImportSzobaButton({
  categoryId,
  categoryName,
  onDone,
}: {
  ...
  async function runImport() {
    setBusy(true);
    try {
      const r = await apiClient.post(`/minicrm/import/szoba/${categoryId}`);
      const data = r.data;
      if (data.skipped) {
        toast.info(`Szoba-import: ${data.skipped}`);
      } else {
        const s = data.stats;
        const relinked = s.projects.relinked ?? 0;
        const unDeleted = s.projects.unDeleted ?? 0;
        const extras: string[] = [];
        if (unDeleted > 0) extras.push(`${unDeleted} visszaállítva`);
        if (relinked > 0) extras.push(`${relinked} átkapcsolva`);
        const projectsPart = `${s.projects.created} új projekt (${s.projects.skipped} már létezett` +
          (extras.length > 0 ? `, ebből ${extras.join(' és ')}` : '') + `)`;
        const summary = `${projectsPart}, task: ${s.tasks}` +
          (s.drone ? `, drón: ${s.drone}` : '') + (s.internalTasks ? `, internal: ${s.internalTasks}` : '');
        toast.success(`${categoryName} kész — ${summary}`);
      }
      setConfirmOpen(false);
      onDone();
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? 'Szoba-import sikertelen');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <>
      <button
        onClick={openConfirm}
        disabled={busy}
        title={`MiniCRM-ből importálja a "${categoryName}" szoba projektjeit + task-okat. Csak admin/CEO.`}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-amber-300 bg-amber-50 text-amber-800 text-xs font-medium hover:bg-amber-100 transition disabled:opacity-50"
      >
        <Download className="w-3.5 h-3.5" />
        {busy ? 'Importálás…' : 'MiniCRM import'}
      </button>
      ...
      {confirmOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="minicrm-szoba-confirm-title"
        >
          <div
            className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4 relative"
          ...
          >
            <button
              ...
              disabled={busy}
              aria-label="Bezárás"
              className="absolute top-3 right-3 w-7 h-7 flex items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-50"
            >
              ×
            </button>
            <h2 id="minicrm-szoba-confirm-title" className="text-lg font-semibold text-gray-900 pr-6">
              MiniCRM import — {categoryName}
            </h2>
            <p className="text-sm text-gray-700 whitespace-pre-line">
              {`Importálod a "${categoryName}" szoba projektjeit a MiniCRM-ből?\n\n` +
                'Ez beimportálja a modul projektjeit + a hozzájuk tartozó task-okat. ' +
                'A meglévő rekordokat NEM duplikálja (idempotens), és a más szobákban lévő ' +
                'projekteket NEM érinti.'}
            </p>
            <div className="flex gap-2 justify-end">
              <button
                type="button"
                onClick={() => setConfirmOpen(false)}
                disabled={busy}
                className="px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50"
              >
                Mégse
              </button>
              <button
